...
if you have problems with installating docker.io on ubuntu try this command:  
...
<pool_id> ---> xxx.factory.shardnet.near WHERE xxx is you pool name
```
near generate-key <pool_id>
```

* Copy the generated keyfile to the ~/.near folder:
```
cp ~/.near-credentials/shardnet/xxx.factory.shardnet.near.json ~/.near/validator_key.json
```

* Edit the keyfile and change `private_key` to `secret_key`
```
nano ~/.near/validator_key.json
```
Use CTRL X to exit & save the changes.

> Note: The account_id in the keyfile must match the staking pool contract name or you will not be able to sign blocks or chunks!

The keyfile must be in the following pattern:
```
{
